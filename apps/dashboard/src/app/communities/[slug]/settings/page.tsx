...
export default function CommunitySettingsPage() {
  ...
  return (
    <div className="p-8">
          ...
          {admins.map((admin) => (
            <div key={admin.membershipId} className="flex items-center justify-between py-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gold-pale flex items-center justify-center text-sm font-semibold text-gold">
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-3">{admin.phone}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                  admin.role === "owner"
                    ? "bg-ink text-white"
                    : "bg-cream text-ink-2 border border-border-subtle"
                }`}>
                ...
                {admin.role === "admin" && (
                  <button
                    ...
                    className="text-xs text-rose hover:text-rose/80 transition disabled:opacity-50"
                  >
      ...
      {/* Add admin */}
      <div className="mt-6 rounded-xl border border-border bg-warm p-6">
        <h2 className="font-serif text-lg font-medium text-ink">Add an admin</h2>
        <p className="mt-1 text-sm text-ink-2">
        ...
        <form onSubmit={handleAddAdmin} className="mt-4">
          <div className="flex gap-2">
            <div className="flex flex-1 rounded-lg border border-border focus-within:border-gold focus-within:ring-1 focus-within:ring-gold overflow-hidden">
              <div className="relative flex items-center">
                <select
                  value={country}
                  onChange={(e) => setCountry(e.target.value)}
                  className="appearance-none bg-cream pl-3 pr-7 py-2.5 text-sm text-ink-2 border-r border-border focus:outline-none cursor-pointer"
                >
              ...
              <input
                ...
                className="flex-1 px-2 py-2.5 text-sm bg-transparent text-ink placeholder:text-ink-3 border-0 focus:outline-none focus:ring-0"
              />
            </div>
            <button
              type="submit"
              disabled={isAdding || phone.length < 10}
              className="rounded-xl bg-ink px-5 py-2.5 text-sm font-semibold text-white hover:bg-ink/90 transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
          ...
          {success && (
            <p className="mt-2 text-sm text-sage">{success}</p>
          )}
        </form>
      </div>
    </div>
